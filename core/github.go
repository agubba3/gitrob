...
func GetUsernameFromEmail(email string, client *github.Client) (string, error) {
	ctx := context.Background()
	emailPtr := new(string)
	*emailPtr = email
	opt := &github.SearchOptions{}

	user, resp, err := client.Search.Users(ctx, *emailPtr, opt)
	if err == nil && resp != nil {
		return *(user.Users[0].Login), err
	} else {
		return "", err
	}
}
